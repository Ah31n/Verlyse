...
/* ================================================================== */
/* 19 · MIR RAZA ALI — the waffle cart. Its lamp keeps burning; behind */
/* it the small lights of a dream light one by one and stay. A memorial*/
/* and a demand: his story cannot become another headline we forget.   */
/* ================================================================== */
function SceneMirRaza({ anim }: { anim: boolean }) {
  const { ref, show } = useShow(anim)
  return (
    <Stage label="his story cannot become another headline">
      <div ref={ref} className="flex flex-col items-center gap-8">
        <svg viewBox="0 0 360 220" className="h-[clamp(180px,30vh,320px)] w-auto" aria-hidden="true">
          {/* the awning */}
          <path d="M112 96 L248 96 L236 76 L124 76 Z" fill="none" stroke="rgba(184,145,70,0.5)" strokeWidth="1.4" />
          {/* the cart */}
          <rect x="120" y="96" width="120" height="64" fill="none" stroke="rgba(248,246,242,0.55)" strokeWidth="1.6" />
          {/* the handle */}
          <path d="M240 120 C 268 116, 276 108, 282 96" fill="none" stroke="rgba(248,246,242,0.45)" strokeWidth="1.4" />
          {/* the wheels */}
          <circle cx="146" cy="176" r="14" fill="none" stroke="rgba(248,246,242,0.5)" strokeWidth="1.6" />
          <circle cx="214" cy="176" r="14" fill="none" stroke="rgba(248,246,242,0.5)" strokeWidth="1.6" />
          {/* the lamp — the dream, still burning */}
          <g className="sig-protect">
            <circle cx="180" cy="120" r="5" fill="#B89146" />
            <circle cx="180" cy="120" r="16" fill="rgba(184,145,70,0.12)" />
          </g>
          {/* the growing lights — the dream that became a brand, one by one */}
          {[60, 84, 276, 300].map((x, i) => (
            <motion.circle
              key={x}
              cx={x}
              cy={176}
              r="3"
              fill="rgba(184,145,70,0.8)"
              initial={anim ? { opacity: 0 } : false}
              animate={show ? { opacity: 1 } : undefined}
              transition={{ delay: 2 + i * 1.6, duration: 1.6, ease: 'easeOut' }}
            />
          ))}
        </svg>
        <p aria-hidden="true" className="max-w-[40ch] text-center font-serif text-lg font-light italic leading-[1.6] text-ivory/70">
...
const SIGNATURES: Record<string, { name: string; ownFin?: boolean; scene: (anim: boolean) => ReactNode }> = {
  ...
  'mir-raza-ali': { name: 'the cart', scene: (anim) => <SceneMirRaza anim={anim} /> },
}
