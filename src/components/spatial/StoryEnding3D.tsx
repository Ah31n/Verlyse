 ...
 *
 * `duration` is the period in seconds and defaults to the house `breathe`
 * constant (5s) from src/lib/motion.ts. It was 1s, which made every ending
 * scene visibly oscillate — the opposite of "ambient, near-still". Scenes that
 * genuinely want a quicker pulse pass an explicit duration and keep it.
 */
function SlowGroup({ children, dy = 0.05, ry = 0.02, duration = DUR.breathe }: { children: ReactNode; dy?: number; ry?: number; duration?: number }) {
  ...
  const render =
    (phase === 'ALIVE' || phase === 'ENTERING') && !reduce && supported ? (
      <Canvas
        ...
        // Same ACES filmic grade as the spatial archive, so an article's ending
        // and the archive it came from are lit by the same world.
        gl={{
          antialias: true,
          alpha: true,
